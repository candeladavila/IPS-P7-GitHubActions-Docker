...
public class PacienteControllerMockMvcIT extends AbstractIntegration {
    ...
    void savePaciente_RecuperaPacientePorId() throws Exception {
        ...
        mockMvc.perform(get("/paciente/{id}", paciente.getId()))
                ...
                .andExpect(jsonPath("$.medico.dni").value("835"));
        
    }

    @Test
    @DisplayName("Listar pacientes de un medico")
    void getPacientesMedico_DevuelveListaDelMedico() throws Exception {
        crearMedico(medico);
        crearPaciente(paciente);

        mockMvc.perform(get("/paciente/medico/{id}", medico.getId()))
                .andExpect(status().isOk())
    ...
    @Test
    @DisplayName("Actualizar paciente asociado a medico")
    void updatePaciente_ModificaPaciente() throws Exception {
        crearMedico(medico);
        crearPaciente(paciente);
    ...
    void deletePaciente_EliminaPacienteExistente() throws Exception {
        crearMedico(medico);
        crearPaciente(paciente);

        mockMvc.perform(delete("/paciente/{id}", paciente.getId()))
                .andExpect(status().isOk());
    }
